...
def find_rest_in_building(building_num, db):
    ...
    conn = sqlite3.connect(db)
    cursor = conn.cursor()
    cursor.execute("SELECT restaurants.name FROM restaurants JOIN buildings ON restaurants.building_id = buildings.id WHERE buildings.building = ?", (building_num,))
    data = cursor.fetchall()
    list = [row[0] for row in data]
    sorted_list = sorted(list, key=lambda x:  get_restaurant_rating(x, db), reverse=True) #check the x[0]
    #lambda statement
    return sorted_list
    pass

def get_restaurant_rating(rest_name, db):
    """
    This helper function accepts the restaurant name and the filename of the database as parameters and returns the rating 
    of the restaurant. If the restaurant is not found in the database, return 0.
    """
    conn = sqlite3.connect(db)
    cursor = conn.cursor()
